...
It can be used for any C project, but it was originally made for the Libspm/CCCP
project. It is licensed under the GNU General Public License v3.0.
 ...
 * mvsp - Move a file or a folder and create the destination folder if it
doesn't exist (`install -D`)
...
#define popcharn(str, pos, s_size)                                             \
  if (pos < s_size) {                                                          \
    memmove(&str[pos], &str[pos + 1], s_size - pos - 1);                       \
    str[s_size - 1] = '\0';                                                    \
  }
...
enum level { INFO, ERROR, WARNING, MEMORY, FATAL };
...
int f_dbg__(int level, int line, const char *function, const char *file,
            char *message, ...);
#define dbg(level, message, ...)                                               \
  f_dbg__(level, __LINE__, __func__, __FILE__, message, ##__VA_ARGS__)
