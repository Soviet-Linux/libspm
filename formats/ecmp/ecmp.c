...
#endif
{
  ...
  void *pairs[][2] = {{"info", parsers[0]},
                      ...
                      {"locations", parsers[9]},
                      {NULL, NULL}};
  void *infodict[][2] = {{"name", &pkg->name},       {"version", &pkg->version},
                         {"type", &pkg->type},       {"url", &pkg->url},
                         {"license", &pkg->license}, {NULL, NULL}};
...
unsigned int parseraw(char *s, char **dest) {
  ...
  // We suppose that our `s` string is dynamically allocated and that it wont be
  // arbitarily freed So we are just going to copy the pointer to it In the last
  // version , we were copying the string to a new buffer Because the `s` string
  // was a buffer that was going to be freed by `getline()`
  *dest = s;
  return strlen(s);
}
...
unsigned int parseinfo(char *s, struct package *dest) {
  ...
  while (*p != '\0') {
    if (*p == ' ') {
      int index = p - s;
      dbg(3, "Removing space '%c']'%c'['%c' at %p (index=%d)", *(p - 1), *p,
          *(p + 1), p, index);
      popchar(s, index);
      p--;
    }
  ...
  *sections = calloc(16, sizeof(section));
  ...
  while ((read = getline(&line, &len, fp)) != EOF) {
    ...
    if (line[0] == '[') {
      ...
      dbg(3, "allocating %d bytes at %p for section %s", 256, sec->buff,
          sec->name);
    ...
    if (bufflen + linelen + 2 >= alloc) {
      ...
      (*sections)[sectionscount - 1]->buff =
          realloc((*sections)[sectionscount - 1]->buff, alloc);
    }
...
#endif
{
  ...
  if (pkg->name != NULL)
    fprintf(ecmp, "name = %s\n", pkg->name);
  if (pkg->version != NULL)
    fprintf(ecmp, "version = %s\n", pkg->version);
  if (pkg->type != NULL)
    fprintf(ecmp, "type = %s\n", pkg->type);
  if (pkg->license != NULL)
    fprintf(ecmp, "license = %s\n", pkg->license);
  if (pkg->url != NULL)
    fprintf(ecmp, "url = %s\n", pkg->url);
