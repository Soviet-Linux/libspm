...
int check(const char *name) {
  ...
  if (pkg.locationsCount == 0) {
    return 2; // Exit code 2 indicates the package is corrupted (no location
              // info)
  }

  // Check the existence of package locations and return the appropriate exit
  // code
...
int check_locations(char **locations, int locationsCount) {
  for (int i = 0; i < locationsCount; i++) {
    ...
    if (access(locations[i], F_OK) != 0) {
      return 3; // Exit code 3 indicates the package is corrupted (some
                // locations are missing)
    }
  }
