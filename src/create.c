...
int create_binary_from_source(const char *spm_path, const char *bin_path) {
  return f_create_binary_from_source(spm_path, bin_path, NULL,
...
int f_create_binary_from_source(const char *src_path, const char *bin_path,
  ...
  PACKAGE_QUEUE[QUEUE_COUNT] =
      pkg.name; // Add the package name to the PKG_QUEUE ARRAY
  QUEUE_COUNT++;
  ...
  */
  ...
  sprintf(file_path, "%s/%s.%s", BUILD_DIR, pkg.name,
          getenv("SOVIET_DEFAULT_FORMAT"));
