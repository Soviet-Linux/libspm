...
int create_table_installed(sqlite3 *db) {
  ...
  int rc = sqlite3_exec(db,
                        "CREATE TABLE IF NOT EXISTS Packages (Name TEXT, "
                        "Version TEXT, Type TEXT, AsDep INT)",
                        0, 0, &err_msg);
...
- int as_dep: Flag indicating whether the package is installed as a dependency
(1 for yes, 0 for no).
...
int store_data_installed(sqlite3 *db, struct package *pkg, int as_dep) {
  ...
  dbg(1, "Inserted %s %s %s -- exit: %d ", pkg->name, pkg->version, pkg->type,
      rc);
...
- sqlite3 *db: An SQLite database handle.
- struct package *pkg: A pointer to a package structure.
- int *as_dep: A pointer to an integer to store whether the package was
installed as a dependency (optional).
...
int retrieve_data_installed(sqlite3 *db, struct package *pkg, int *as_dep) {
  ...
  while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    ...
    if (as_dep != NULL)
      *as_dep = sqlite3_column_int(stmt, 2);
  }
...
int retrieve_data_repo(sqlite3 *db, struct package *pkg, char **format,
  ...
  // Prepare the SQL query
  const char *sql =
      "SELECT Version, Type, Format, Section FROM Packages WHERE Name = ?";
  rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
  if (rc != SQLITE_OK) {
  ...
  while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    ...
    pkg->type = strdup((char *)sqlite3_column_text(stmt, 1));
    if (format != NULL)
      (*format) = strdup((char *)sqlite3_column_text(stmt, 2));
    if (section != NULL)
      (*section) = strdup((char *)sqlite3_column_text(stmt, 3));
  }
...
int print_all_data(sqlite3 *db) {
  ...
  while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    msg(INFO, "%s ==> %s - %s", sqlite3_column_text(stmt, 0),
