...
int f_install_package_source(const char *spm_path, int as_dep,
  ...
  if (pkg.dependencies != NULL && pkg.dependenciesCount > 0 &&
      strlen(pkg.dependencies[0]) > 0) {
    dbg(1, "Checking dependencies...");
    check_dependencies(pkg.dependencies, pkg.dependenciesCount);
  }

  // Checking makedeps
  if (pkg.makedependencies != NULL && pkg.makedependenciesCount > 0 &&
      strlen(pkg.makedependencies[0]) > 0) {
  ...
  sprintf(legacy_dir, "%s/%s-%s", getenv("SOVIET_MAKE_DIR"), pkg.name,
          pkg.version);
  ...
  pkg.locationsCount =
      get_locations(&pkg.locations, getenv("SOVIET_BUILD_DIR"));
  ...
  sprintf(file_path, "%s/%s.%s", getenv("SOVIET_SPM_DIR"), pkg.name,
          getenv("SOVIET_DEFAULT_FORMAT"));
...
*/
__attribute__((unused)) int uncompress_binary(const char *bin_path,
                                              const char *dest_dir);
